...
@implementation ApplePlay

int count;

+(BOOL)PayGems:(NSString *)str {
    
    NSLog(@"js调用Objective-c ApplePlay中的PayGems方法");
    count = 1;
    ApplePlay  *objectName = [[ApplePlay alloc]init] ;
    [objectName testPay];
    return true;
...
- (void)verifyPruchase {
    ...
    if (dict != nil) {
        ...
        NSLog(@"objective-c调用js Hall类中的applePaySuccess方法");
        if(count){
            count = 0;
            ScriptingCore::getInstance()->evalString("cc.find('Canvas').getComponent('Hall').applePaySuccess();");
        }
